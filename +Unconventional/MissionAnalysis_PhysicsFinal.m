...
%tripRange = 500e3;   % [m] pick one
%tripRange = 10000e3;   % [m] pick one
%M_TO = ADP.MTOM;           % optional → will default to ADP.MTOM

%[BlockFuel,TripFuel,ResFuel,Mf_TOC,MissionTime,cruise_FL] = MissionAnalysis_PhysicsFinal(ADP, tripRange, M_TO);


function [BlockFuel,TripFuel,ResFuel,Mf_TOC,MissionTime,cruise_FL] = MissionAnalysis_PhysicsFinal(ADP,tripRange,M_TO)
%MISSIONANALYSIS Detailed stepped mission analysis
%
% Outputs:
%   BlockFuel   - total fuel from mission start to final taxi-in [kg]
%   TripFuel    - design mission fuel only (taxi out -> destination descent) [kg]
%   ResFuel     - reserve/alternate/contingency/taxi-in fuel [kg]
%   Mf_TOC      - mass fraction at top of climb [-]
%   MissionTime - main design mission time [s]
%   cruise_FL   - selected cruise flight level [-]
...
% keep these off during sizing / optimisation loops
verbose  = false;
makePlots = false;
...
% ---------------------- Pull from ADP ----------------------
m0 = M_TO;          % <-- use function input, not ADP.MTOM
S   = ADP.WingArea;
...
T2W_crit = ((1.21/(g*rho0*Cl_TO*s_g))*(m0*g/S)) + ...
           (0.5*(1/L2D_TO)) + (0.5*mu);
...
FuelTaxi = TSFC_idle * T_idle * TaxiTime_out;

m_after_taxi = m0 - FuelTaxi;
...
ReserveFuelFrac_MTOM = Reserve_m_TOTAL / M_TO;
%ADP.Mf_res  = Reserve_m_TOTAL / M_TO;
...
TaxiTime_in = 15*60;       % [s] reasonable assumption

T_idle_land = IdleFrac_land * T_TO;

TSFC_idle_land = ADP.Engine.TSFC(0,0);

FuelTaxi_in = TSFC_idle_land * T_idle_land * TaxiTime_in;

% --- FINAL MASS ---
% m_final = m_landing - FuelTaxi_land;
m_final = m_landing_dest - FuelTaxi_in;

% --- MASS FRACTION ---
% Mfn_landing_taxi = m_final / m_landing;
Mfn_landing_taxi = m_final / m_landing_dest;

% --- TIME ---
t_taxi_in = TaxiTime_in;
...
m5 = m_after_loiter;                      % after alternate mission
...
% main mission ends at destination descent
m_trip_end = m_end_cruise - Descent_DeltaM;

% TripFuel  = M_TO - m_trip_end;
% BlockFuel = M_TO - m_final;
% ResFuel   = BlockFuel - TripFuel;

% ================================
% TRIP FUEL (YOUR DEFINITION: FULL MISSION WITHOUT RESERVES)
% ================================

TripFuel = FuelTaxi + FuelTO + ...
           Climb_m_TOTAL + Cruise_DeltaM + Descent_DeltaM + ...
           FuelTaxi_in;

% ================================
% CONTINGENCY (FIXED PROPERLY)
% ================================

Fuel_cont_3pct = 0.03 * TripFuel;
Fuel_cont = max(Fuel_cont_time, Fuel_cont_3pct);

% ================================
% RESERVE FUEL (ONLY EXTRA MISSION)
% ================================

ResFuel = Fuel_cont + Reserve_m_TOTAL + Fuel_loiter;

% ================================
% BLOCK FUEL (TOTAL ONBOARD AT START)
% ================================

BlockFuel = TripFuel + ResFuel;

% true top-of-climb mass fraction
Mf_TOC = m4c / M_TO;

% use main design mission time only
MissionTime = TaxiTime_out + t_TO + Climb_t_TOTAL + Cruise_t_TOTAL + Descent_t_TOTAL + TaxiTime_in;

% cruise flight level
cruise_FL = round((h4/ft2m)/100, 0);

% update ADP state for downstream sizing code
ADP.Mf_TOC = Mf_TOC;
ADP.Mf_res = ResFuel / M_TO;
