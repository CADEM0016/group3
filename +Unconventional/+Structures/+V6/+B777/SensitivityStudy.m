...
for k = 1:numel(ARs)
    ...
    Gk      = Unconventional.Structures.V6.B777.WingGeometry(ak, tlar, loc);
    m_emp(k) = Unconventional.Structures.V6.B777.EmpiricalMass(ak, tlar, loc, Gk).m_total;
...
for k = 1:numel(MTOMs)
    ...
    ak.MTOM = MTOMs(k);
    Gk      = Unconventional.Structures.V6.B777.WingGeometry(ak, tlar, loc);
    m2_emp(k) = Unconventional.Structures.V6.B777.EmpiricalMass(ak, tlar, loc, Gk).m_total;
...
for k = 1:numel(spans)
    ...
    ak.Span = spans(k);
    Gk      = Unconventional.Structures.V6.B777.WingGeometry(ak, tlar, loc);
    m3_emp(k) = Unconventional.Structures.V6.B777.EmpiricalMass(ak, tlar, loc, Gk).m_total;
...
G_dp    = Unconventional.Structures.V6.B777.WingGeometry(adp, tlar, loc);
m5_emp  = cellfun(@(m) Unconventional.Structures.V6.B777.EmpiricalMass(adp, tlar, loc, G_dp, m).m_total, mats);
...
E_dp     = Unconventional.Structures.V6.B777.EmpiricalMass(adp, tlar, loc, G_dp, 'Al');
...
for k = 1:3
    Lk = Unconventional.Structures.V6.B777.LoadDistribution(adp, tlar, loc, G, cases{k});
    Sk = Unconventional.Structures.V6.B777.SMT(loc, G, Lk);
...
function m = run_II5(adp, tlar, loc, lc, mat)
    Gk  = Unconventional.Structures.V6.B777.WingGeometry(adp, tlar, loc);
    Lk  = Unconventional.Structures.V6.B777.LoadDistribution(adp, tlar, loc, Gk, lc);
    Sk  = Unconventional.Structures.V6.B777.SMT(loc, Gk, Lk);
    Wk  = Unconventional.Structures.V6.B777.WingboxSizing(loc, Gk, Sk, mat);
    MBk = Unconventional.Structures.V6.B777.MassBuildup(adp, loc, Gk, Wk);
    m   = MBk.m_total;
end
