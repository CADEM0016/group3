...
% Buckling-governed minimum skin thickness - constant along span
...
for i = 1:G.N
    ...
    % Spar caps from bending - idealised 4-cap box, skins carry 40%
    if h > 1e-6 && Mi > 0
        A_cap(i) = 0.60 * Mi / (2 * mat.sig_all * h);
    end
    A_cap(i) = max(A_cap(i), mat.A_cap_min);

    % Skin from Bredt-Batho torsion; governed by buckling outboard
    if Ae > 1e-8 && Ti > 0
        t_skin(i) = Ti / (2 * Ae * mat.tau_all);
    end
    t_skin(i) = max([t_skin(i), t_buckle, mat.t_min]);

    % Web from Jourawski shear  (tau_max = 1.5 × tau_avg)
    if h > 1e-6 && Qi > 0
