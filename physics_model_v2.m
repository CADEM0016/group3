function [BlockFuel, TripFuel, ResFuel, Mf_TOC, ...
          ...
          MissionAnalysisPhysics(ADP, tripRange, M_TO)
...
cruisePath = fullfile(baseDir, 'cruise_lookup_table.mat');

Slookup = load(cruisePath);
Results = Slookup.Results;

function CD = lookupCD_fromTable(CL_req, CL_tab, CD_tab)

    [CL_tab, idx] = sort(CL_tab);
    CD_tab = CD_tab(idx);

    % remove duplicate CL values if they exist
    [CL_tab, uniqIdx] = unique(CL_tab, 'stable');
    CD_tab = CD_tab(uniqIdx);

    CL_req = max(min(CL_req, max(CL_tab)), min(CL_tab));

    CD = interp1(CL_tab, CD_tab, CL_req, 'linear');
end
...
[m_current, Seg] = flyVerticalSegment(ADP, m_current, ...
    ...
    Results, Slookup, span_lookup, rootChord_lookup);
Mission.Climb1 = Seg;
...
%% ============================================================
[m_current, Seg] = flyVerticalSegment(ADP, m_current, ...
    20000 / SI.ft, ADP.TLAR.Alt_cruise, 500 / SI.ft, ...
...
[m_current, Seg] = flyCruiseSegment(ADP, m_current, ...
    RangeCruiseTarget, M_cruise, ADP.TLAR.Alt_cruise, 'MainCruise', ...
    Results, Slookup, span_lookup, rootChord_lookup);
Mission.Cruise = Seg;
...
%% ============================================================
[m_current, Seg] = flyVerticalSegment(ADP, m_current, ...
    ADP.TLAR.Alt_cruise, 20000 / SI.ft, 500 / SI.ft, ...
...
function [m_out, Seg] = flyVerticalSegment(ADP, m_in, h_start, h_end, dh, ...
    speedMode, speedValue, roc, segName, Results, Slookup, span_lookup, rootChord_lookup)
...
function [CL_tab, CD_tab] = getLookupSlice(Results, Slookup, span, rootChord, Mach, alt)

    span_use = Slookup.spanVec(nearestIndex(Slookup.spanVec, span));
    mach_use = Slookup.machVec(nearestIndex(Slookup.machVec, Mach));
    root_use = Slookup.rootChordVec(nearestIndex(Slookup.rootChordVec, rootChord));
    alt_use  = Slookup.altVec(nearestIndex(Slookup.altVec, alt));

    idx = [Results.WingSpan]  == span_use & ...
          [Results.Mach]      == mach_use & ...
          [Results.RootChord] == root_use & ...
          [Results.Altitude]  == alt_use;

    if ~any(idx)
        error('No lookup-table data found for this condition.');
    end

    CL_tab = [Results(idx).CL];
    CD_tab = [Results(idx).CD];
end
...
for i = 1:nSteps
    ...
    [CL_tab, CD_tab] = getLookupSlice( ...
    Results, Slookup, span_lookup, rootChord_lookup, Mach, h_mid);


    CD = lookupCD_fromTable(CL, A, CL_tab, CD_tab);
...
for i = 1:nSteps
    ...
    CD = lookupCD_fromTable(CL, CL_tab, CD_tab);
