...
payload_design_t = 123;                  % tonnes
payload_design   = payload_design_t*1000;
...
for i = 1:nRoutes
    ...
    if dist_km(i) <= R_design_km
        ...
        payload_split_t(end+1) = payloads_routes_t(i);
        
    else
        % split into 2 legs
        leg_dist = dist_km(i)/2;
        
        % both legs at design payload
        dist_split_km(end+1) = leg_dist;
        payload_split_t(end+1) = payload_design_t;
...
% envelope
plot(ranges_km, payloads_t, '-ob','LineWidth',2,'DisplayName','Envelope')
...
% split routes
plot(dist_split_km, payload_split_t, 'rd', ...
    'MarkerFaceColor','r', ...
    'DisplayName','Split Routes (123t ops)')

% design point
plot(R_design_km, payload_design_t, 'bp', ...
    'MarkerSize',12, ...
    'LineWidth',2, ...
    'DisplayName','Design Point')

% vertical design range line
